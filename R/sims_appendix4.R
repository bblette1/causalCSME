...
simulator <- function(trial, sigma_me) {
  ...
  L2 <- rnorm(n, L2_mean, 0.3)
  A <- rnorm(n, 2 + 0.9*L1 - 0.6*L2, 1.1)
  Y_prob <- exp(-2.5 + beta1_true*A - 0.8*L1 - 0.4*L2)
  Y_prob[Y_prob > 1] <- 1 # failsafe since running 2000 sims
  Y <- rbinom(n, 1, Y_prob)
  ...
  mod <- glm(Y ~ Astar + L2, data = data, family = binomial(link = "log"))
  ...
  eefun_csme_gform <- function(data) {
    ...
    condexp <- function(beta0, beta1, beta2, sigma_ep) {
      (beta0 + (beta1)*delta(beta1, sigma_ep) + beta2*L2) /
        (1 + ((beta1)^2)*sigma_me / sigma_ep)[[1]]
    }
    condvar <- function(beta1, sigma_ep) {
      sigma_ep / (1 + ((beta1)^2)*sigma_me / sigma_ep)[[1]]
    }
    function(theta) {
      c((Y - condexp(theta[1], theta[2], theta[3], theta[4])),
        ...
        theta[4] - theta[4]*
          (Y - condexp(theta[1], theta[2], theta[3], theta[4]))^2 /
          condvar(theta[2], theta[4])
      )
    }
  }
  ...
  while(failed == TRUE & j < 6) {
    ...
    failed <- FALSE
    startvec <- c(coef(mod), sigma(mod)^2)*(j == 1) +
      c(coef(mod) + rnorm(3, 0, j/5), sigma(mod)^2)*(j > 1)
    results_csme_gform <-
    ...
    if (failed == FALSE) {
      if (abs(coef(results_csme_gform)[2]) > 3) { failed <- TRUE }
    }
    j <- j + 1

  }

  bias_gform_ps <- coef(results_csme_gform)[2] - true_effect
  se_gform_ps <- sqrt(vcov(results_csme_gform)[2, 2])
  coverage_gform_ps <-
    1*(coef(results_csme_gform)[2] - 1.96*se_gform_ps < true_effect &
         coef(results_csme_gform)[2] + 1.96*se_gform_ps > true_effect)
  ...
  wmod <- glm(Y ~ Astar, weights = sw, data = data,
              family = binomial(link = "log"), start = c(-3, guess))
