...
Now suppose instead that the propensity score model(s) are correctly specified such that $\hat{\zeta} = \zeta$ for the MLE of the model parameters $\hat{\zeta}$. Leveraging the weighted least squares estimator for the outcome regression parameters, the DR-CSM estimator can be rewritten as
\begin{equation*}
