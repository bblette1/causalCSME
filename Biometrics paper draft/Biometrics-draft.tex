...
An example directed acyclic graph (DAG) for this setup is presented in Figure~\ref{fig:one}. This corresponds to a problem with four exposures, one each of the following: unconfounded and correctly measured, unconfounded and mismeasured, confounded and correctly measured, and confounded and mismeasured. Many existing methods focus on only mismeasured exposures or on a set of correctly measured exposures with one additional mismeasured exposure; the methods developed in this paper provide a more broadly usable modeling framework where all four types of exposures can be studied simultaneously. In addition, the DAG implies non-differential measurement error with respect to the outcome, i.e., each exposure error is independent of the outcome because the corresponding measured exposure ($A^{*}(1)$ or $A^{*}(4)$) is a collider on all paths between them.
